...
Note that if the `getColumnIdentity` function returns group value that is an object, you should also specify a custom group cell template using the `TableGroupRow` plugin's `groupCellTemplate` property as demonstrated in the following example:
