...
addedRows | Array&lt;any&gt; | | Specifies created but not committed rows data.
defaultAddedRows | Array&lt;any&gt; | | Specifies rows data initially added to the `addedRows` array in the uncontrolled mode.
onAddedRowsChange | (addedRows: Array&lt;any&gt;) => void | | Handles adding or removing a row from the `addedRows` array.
changedRows | { [key: string]: any } | | Specifies changed but not committed rows data.
...
Field | Type | Description
------|------|------------
added? | Array&lt;any&gt; | An array of row data to be created.
changed? | { [key: number &#124; string]: any } | An associative array storing changes made to existing data. Each array item specifies changes made to a row data. The item's key specifies the associated row's data ID.
...
changeRow | Action | ({ rowId: number &#124; string, change: any }) => void | Adds an item representing changes made to an exsiting row data to the `changedRows` array.
