...
changeSortingDirection | ({ keepOther: boolean, remove: boolean }) | Changes column sort direction. Keeps existing sorting if `keepOther` is set to `true`. Clears existing sorting for the current column if `remove` is set to true.
