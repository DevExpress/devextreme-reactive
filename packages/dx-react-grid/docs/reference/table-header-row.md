...
changeSortingDirection | ({ keepOther: boolean, cancel: boolean, scope: Array&lt;String&gt; }) | Changes column sort direction. Keeps the existing sorting if `keepOther` is set to `true`. Cancels sorting by the current column if `cancel` is set to true. `scope` is an array which contains `columnName` among that sorting is carried out.
