...
describe('GroupPanel utils', () => {
  describe('#getGroupCellTargetIndex', () => {
    it('should return target index correctly when adding new group and all elements are on the same line', () => {
      const geometries = [
    ...
    it('should return target index correctly when adding a new group and elements are on multiple lines', () => {
      const geometries = [
    ...
    it('should return target index correctly when there are no initial geometries', () => {
      expect(getGroupCellTargetIndex([], -1, { x: 10, y: 10 }))
        .toBe(0);
    });
  });
});
