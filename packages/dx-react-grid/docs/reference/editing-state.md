...
changed? | { [key: number &#124; string]: Object } | An associative array holding changes made to existing rows. Each array item is an object that specifies changes made to a row. The item's key specifies the ID of the associated row.
...
changedRows | Getter | { [key: string]: Object } | An associative array holding changes made to existing rows. Each array item is an object that specifies changes made to a row. The item's key specifies the ID of the associated row.
