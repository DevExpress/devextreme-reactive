...
* **react-grid:**
Since we've extracted a sorting controll component, the `getMessage` property, that returns  the sort label’s text, has been moved from the `TableHeaderRow.CellProps` interface to the `TableHeaderRow.SortLabelProps` one.
