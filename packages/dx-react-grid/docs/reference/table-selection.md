...
setRowsSelection | Action | ({ rowIds: Array&lt;number &#124; string&gt; }) => void | Selects/deselects rows. To select/deselect a single row, use an array with a single item as the `rowIds` argument
