...
Field | Type | Description
------|------|------------
allowSorting | boolean | Specifies whether an end-user can sort data by columns contained in the grouping panel.
...
changeSortingDirection | ({ keepOther: boolean, cancel: boolean, columnName: string }) => void | Changes the direction of sorting by the specified column. Keeps the current sorting state if `keepOther` is set to true. Cancels sorting by the specified column if `cancel` is set to true.
...
column | [Column](#column) | Specifies the column associated with the item.
draft | boolean | Specifies whether the item should be rendered for the preview.
allowSorting | boolean | Specifies whether an end-user can sort data by the column while it is contained in the grouping panel.
sortingDirection? | 'asc' &#124; 'desc' | Specifies the sorting direction.
changeSortingDirection | ({ keepOther: boolean, cancel: boolean, columnName: string }) => void | Changes the direction of sorting by the specified column. Keeps the current sorting state if `keepOther` is set to true. Cancels sorting by the current column if `cancel` is set to true.
